@extends('layouts.app')

@section('content')

<div class="panel panel-default">
    <div class="panel-heading clearfix">

        <span class="pull-left">
            <h4 class="mt-5 mb-5">{{ isset($title) ? $title : 'Chitiet Pdg Cd' }}</h4>
        </span>

        <div class="pull-right">

            <form method="POST" action="{!! route('chitiet_pdg_cds.chitiet_pdg_cd.destroy', $chitietPdgCd->ID) !!}" accept-charset="UTF-8">
            <input name="_method" value="DELETE" type="hidden">
            {{ csrf_field() }}
                <div class="btn-group btn-group-sm" role="group">
                    <a href="{{ route('chitiet_pdg_cds.chitiet_pdg_cd.index') }}" class="btn btn-primary" title="Show All Chitiet Pdg Cd">
                        <span class="glyphicon glyphicon-th-list" aria-hidden="true"></span>
                    </a>

                    <a href="{{ route('chitiet_pdg_cds.chitiet_pdg_cd.create') }}" class="btn btn-success" title="Create New Chitiet Pdg Cd">
                        <span class="glyphicon glyphicon-plus" aria-hidden="true"></span>
                    </a>
                    
                    <a href="{{ route('chitiet_pdg_cds.chitiet_pdg_cd.edit', $chitietPdgCd->ID ) }}" class="btn btn-primary" title="Edit Chitiet Pdg Cd">
                        <span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>
                    </a>

                    <button type="submit" class="btn btn-danger" title="Delete Chitiet Pdg Cd" onclick="return confirm(&quot;Click Ok to delete Chitiet Pdg Cd.?&quot;)">
                        <span class="glyphicon glyphicon-trash" aria-hidden="true"></span>
                    </button>
                </div>
            </form>

        </div>

    </div>

    <div class="panel-body">
        <dl class="dl-horizontal">
            <dt>P H I E U D A N H G I A  C H I D O A N</dt>
            <dd>{{ optional($chitietPdgCd->PhieudanhgiaChidoan)->TEN_PDGCD }}</dd>
            <dt>N O I D U N G  P D G</dt>
            <dd>{{ optional($chitietPdgCd->NoidungPdg)->TEN_NDPDG }}</dd>
            <dt>D I E M  D U Y E T  P D G C D</dt>
            <dd>{{ $chitietPdgCd->DIEM_DUYET_PDGCD }}</dd>
            <dt>D I E M  C D  T U D A N H G I A</dt>
            <dd>{{ $chitietPdgCd->DIEM_CD_TUDANHGIA }}</dd>
            <dt>G H I C H U  P D G C D</dt>
            <dd>{{ $chitietPdgCd->GHICHU_PDGCD }}</dd>

        </dl>

    </div>
</div>

@endsection